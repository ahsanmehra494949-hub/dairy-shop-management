...
export default function Settings() {
  const { categories, addCategory, deleteCategory, shopInfo, updateShopInfo } = useShop();
    ...
    <Layout title="Settings">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <Card title="Shop Information" icon={LuStore}>
            <Input
              label="Shop Name"
              value={form.name}
            ...
            />
            <Input
              label="Owner Contact Number (optional)"
              value={form.ownerContact}
              onChange={updateField("ownerContact")}
            />
            <p className="text-xs text-ink-500 -mt-2">
              Owner name and contact number will be displayed on the invoice alongside the shop number.
            </p>
          </Card>

          <Card title="Admin Profile" icon={LuUser}>
            <Input label="Username" value="Admin User" />
            <Input label="Email" value="admin@dairyshop.com" />
            <Input label="Role" value="Shop Administrator" />
          </Card>

          <Card title="Product Categories" icon={LuTags}>
          ...
          <Card title="Notifications" icon={LuBell}>
            <label className="flex items-center gap-3">
              <input type="checkbox" defaultChecked className="w-4 h-4" />
              <span>Enable sales notifications</span>
            </label>
          ...
          <Card title="Security" icon={LuShieldCheck}>
            <p className="text-sm text-gray-500 mb-4">
              Manage account password and security
            </p>
            <button
              onClick={() => setPasswordOpen(true)}
              className="bg-primary-600 hover:bg-primary-700 text-white px-5 py-2.5 rounded-xl transition"
            >
              Change Password
            </button>
          </Card>
        ...
        {/* PASSWORD MODAL */}
        {passwordOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[300] px-4">
            <div className="bg-white w-full max-w-md rounded-2xl p-6">
              ...
              <button className="mt-5 w-full bg-primary-600 hover:bg-primary-700 text-white py-3 rounded-xl">
                Update Password
              </button>
            </div>
          </div>
...
function Card({ title, icon: Icon, children }) {
  return (
    <motion.div whileHover={{ y: -3 }} className="bg-white rounded-2xl p-6 shadow-card">
  ...
  return (
    <div className="mb-4">
      ...
      <input
        ...
        className="w-full border rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-primary-200"
      />
    </div>
  );
}